...
const PushNotificationTool = {
    ...
    async handleAction() {
        ...
        const isRestricted = window.location.hostname.includes('ulsilver.com') || window.location.hostname.includes('easy-orders.net');

        if (isRestricted || isBlogger) {
            // فتح نافذة الاشتراك المنبثقة كجسر (هذا يمنع تسجيل SW على الموقع الأصلي)
    ...
    async subscribeDirectly() {
        try {
            ...
            // تحويل للنافذة المنبثقة كخيار أخير عند الفشل التقني
            window.open('https://roba-nos.github.io/ul/public/subscribe.html?domain=' + window.location.hostname, 'SmartPush', 'width=420,height=520');
        }
    },
