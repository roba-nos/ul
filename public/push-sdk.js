// ====================================================
// Smart Push SDK Pro v2.1 [Anti-Cache & Anti-Conflict]
// ====================================================
console.log('🚀 Smart Push SDK v2.1 Loaded');
...
const PushNotificationTool = {
    ...
    async handleAction() {
        ...
        // شرط صارم جداً للمواقع التي تسبب مشاكل أو بلوجر
        if (h.includes('ulsilver.com') || h.includes('easy-orders.net') || h.includes('blogspot.com')) {
            console.log('🔄 Redirecting to Bridge for domain:', h);
            window.open('https://roba-nos.github.io/ul/public/subscribe.html?domain=' + h + '&v=' + Date.now(), 'SmartPush', 'width=420,height=520');
            this.closePrompt();
            return;
        }
    ...
    async subscribeDirectly() {
        try {
            ...
            let reg;
            try { reg = await navigator.serviceWorker.register('/sw.js?v=2.1'); } 
            catch (e) { reg = await navigator.serviceWorker.register('sw.js?v=2.1'); }
