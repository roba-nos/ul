// ====================================================
// Smart Push SDK
// ====================================================
const SUPABASE_FUNCTIONS_URL = 'https://rtcnheqqzvvdbrdmpzsr.supabase.co/functions/v1';
const SUBSCRIBE_PAGE_URL     = 'https://roba-nos.github.io/ul/public/subscribe.html';
const SUPABASE_ANON_KEY      = 'sb_publishable_urB1hflL5FDePpONRZ--EA_5b8jmxuz';

const PushNotificationTool = {

  init(options = {}) {
    this.options = {
      showBell: options.showBell !== undefined ? options.showBell : true,
      autoAsk: options.autoAsk || false,
      ...options
    };

    if (this.options.showBell) {
      this.addBellIcon();
    }

    if (this.options.autoAsk) {
      this.setupAutoAsk();
    }
  },

  setupAutoAsk() {
    const askOnce = () => {
      this.openSubscriptionPopup();
      document.removeEventListener('click', askOnce);
    };
    document.addEventListener('click', askOnce);
  },

  askPermission() {
    this.openSubscriptionPopup();
  },

  addBellIcon() {
    if (document.getElementById('push-bell-icon')) return;

    const bell = document.createElement('div');
    bell.id = 'push-bell-icon';
    bell.innerHTML = '🔔';
    Object.assign(bell.style, {
      position: 'fixed',
      bottom: '20px',
      right: '20px',
      width: '50px',
      height: '50px',
      background: 'linear-gradient(135deg, #6366f1, #ec4899)',
      borderRadius: '50%',
      display: 'flex',
      justifyContent: 'center',
      alignItems: 'center',
      color: 'white',
      fontSize: '22px',
      cursor: 'pointer',
      boxShadow: '0 4px 15px rgba(99,102,241,0.5)',
      zIndex: '999999',
      transition: 'transform 0.3s',
    });
    bell.onmouseover = () => bell.style.transform = 'scale(1.15)';
    bell.onmouseout  = () => bell.style.transform = 'scale(1)';
    bell.onclick     = () => this.openSubscriptionPopup();
    document.body.appendChild(bell);
  },

  openSubscriptionPopup() {
    const domain = window.location.hostname || 'unknown';
    const w = 420, h = 520;
    const left = (screen.width  / 2) - (w / 2);
    const top  = (screen.height / 2) - (h / 2);
    window.open(
      `${SUBSCRIBE_PAGE_URL}?domain=${domain}`,
      'SmartPush',
      `width=${w},height=${h},top=${top},left=${left}`
    );
  }
};

// تشغيل تلقائي
if (document.readyState === 'complete') {
  PushNotificationTool.init();
} else {
  window.addEventListener('load', () => PushNotificationTool.init());
}

window.PushNotificationTool = PushNotificationTool;
